...
const EMPTY_VALUES: ProcessStepFormValues = { title: "", description: "", order: 0 };

export function ProcessStepFormModal({
  isOpen,
  initialValues,
  onClose,
  onSaved,
  ...
  isOpen: boolean;
  initialValues: ProcessStepFormValues | null;
  onClose: () => void;
  onSaved: (wasEditing: boolean) => void;
}) {
  const [values, setValues] = useState<ProcessStepFormValues>(initialValues ?? EMPTY_VALUES);
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(
        isEditing ? `/api/process-steps/${initialValues!.id}` : "/api/process-steps",
        {
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <label className="flex flex-col gap-1.5 text-sm">
                ...
                <span className="text-xs text-foreground/50">
                  Lower numbers appear first. The step number shown on the site is derived from
                  this order, not stored separately.
                </span>
              </label>
