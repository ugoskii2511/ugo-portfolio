...
const EMPTY_VALUES: ServiceCategoryFormValues = {
  title: "",
  description: "",
  icon: "code",
  order: 0,
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(
        isEditing ? `/api/service-categories/${initialValues!.id}` : "/api/service-categories",
        {
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <label className="flex flex-col gap-1.5 text-sm">
                ...
                <select
                  value={values.icon}
                  onChange={(event) =>
                    setValues((v) => ({ ...v, icon: event.target.value as ServiceIcon }))
                  }
                  className="rounded-lg border border-border-subtle bg-surface px-3.5 py-2.5 outline-none ring-primary/40 transition focus:ring-2"
                >
                  {SERVICE_ICON_KEYS.map((icon) => (
